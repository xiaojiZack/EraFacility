;(function () {
	...
	class Trait$1 extends Talent {
	  static init() {
	    D.traits.forEach((obj) => {
	      ...
	      obj.id = id;
	      Trait$1.data[id] = new Trait$1(obj);
	    });
	    console.log(Trait$1.data);
	  }
	  static set(type, name) {
	    const traitdata = Object.values(Trait$1.data).filter((trait) => {
	  ...
	  static list(type) {
	    return Object.values(Trait$1.data).filter((trait) => {
	...
	function InitTraitsConflict() {
	  return __async$2(this, null, function* () {
	    Object.values(Trait$1.data).forEach((trait) => {
	      if (trait.conflict) {
	...
	function initBodyObj(body) {
	  ...
	  const fillObj = function(obj, key, parent) {
	    if (groupmatch(key, "group", "option", "sens", "size", "config", "setting", "tags", "produce", "conduct", "capacity"))
	      return;
	    if (!obj.name)
	      obj.name = key;
	...
	function InitSpecies() {
	  ...
	  xml.forEach((value, key) => {
	    if (key.includes("Species")) {
	      let obj = value.race;
	      fixLanArr(obj);
	      MySpecies.data[obj.id] = new MySpecies(obj);
	    }
	  });
	  slog("log", "All Species Loaded: ", MySpecies.data);
	}
	...
	class OrganVessel {
	    ...
	    Object.keys(this.conduct).forEach((ck) => {
	      ...
	      if (cond.inflateRequire ? this.inflateLv >= cond.inflateRequire : true) {
	        ...
	        Object.keys(this.liquid).forEach((lk) => {
	          var cr = ck == "absorb" ? this.leakRate * timeRate : rate;
	  ...
	  Produce(timeRate, which = "all") {
	    ...
	    if (which == "all") {
	      this.FixProduce();
	      Object.keys(this.produce).forEach((k) => {
	        var pl2 = p[k].fixSpeed * timeRate;
	        if (p[k].produceType == "day")
	          l[k] = l[k] ? l[k] + pl2 : pl2;
	      });
	      this.UpdataVessel();
	    } else {
	      if (Object.keys(p).includes(which)) {
	        ...
	        slog("warn", "this organ can not produce" + which);
	    }
	  }
	}
	...
	function initCycle(chara) {
	  var _a;
	  const r = (_a = MySpecies) == null ? void 0 : _a.data[chara.species];
	...
	class MyCreature extends Creature {
	  ...
	  randomTrait() {
	    ...
	    Object.keys(Tl).forEach((Tid) => {
	      let T = Tl[Tid];
	      if (T.group == "personality")
	        temp.push([T.name[1], 100 * T.rate]);
	      else if (T.group == "fallen")
	        ;
	      else
	        pool.push(T);
	    });
	    let personality = Tl[maybe(temp)];
	    tryTrait.push(personality);
	    pool.forEach((T) => {
	      if (random(100) < 100 * T.rate)
	        tryTrait.push(T);
	    });
	...
	class Chara extends MyCreature {
	  static get(charaId) {
